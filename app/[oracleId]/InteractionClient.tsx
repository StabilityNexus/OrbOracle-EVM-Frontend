...
type PriceHistoryResult = readonly [
  readonly bigint[],
  readonly bigint[],
  readonly bigint[],
];
...
export default function OracleInteractionPage() {
  ...
  const [, setTokenAllowance] = useState<string>("0");
  ...
  const [quorumPercentage, setQuorumPercentage] =
    useState<string>("Loading...");
  const [operationLockPeriod, setOperationLockPeriod] =
    useState<string>("Loading...");
  const [withdrawalLockPeriod, setWithdrawalLockPeriod] =
    useState<string>("Loading...");
  ...
  const [lastOperationTimestamp, setLastOperationTimestamp] =
  ...
  const updatePriceDisplays = useCallback(
  ...
  const oracleAddress = isHexAddress(oracleParam)
    ? (oracleParam as `0x${string}`)
    : null;
  ...
  const { data: tokenAllowanceData } = useReadContract({
    ...
    args:
      userAddress && oracleAddress ? [userAddress, oracleAddress] : undefined,
    query: {
      enabled: !!weightTokenAddress && !!userAddress && !!oracleAddress,
    },
  });

  const weightTokenSymbol = useMemo(
    () => (weightTokenSymbolData ? String(weightTokenSymbolData) : "WEIGHT"),
    [weightTokenSymbolData],
  );
  const weightTokenDecimals = useMemo(() => {
    if (
      weightTokenDecimalsData === undefined ||
      weightTokenDecimalsData === null
    ) {
      return 18;
    }
  ...
  const weightTokenAddressString = useMemo(
    () => (weightTokenAddress ? String(weightTokenAddress) : "—"),
    [weightTokenAddress],
  );
  const canCopyWeightTokenAddress = useMemo(
    () =>
      weightTokenAddressString.startsWith("0x") &&
      weightTokenAddressString.length === 42,
  ...
  useEffect(() => {
    ...
    if (lockedTokensData !== undefined || unlockedTokensData !== undefined) {
      const locked = lockedTokensData
        ? BigInt(lockedTokensData as bigint)
        : BigInt(0);
      const unlocked = unlockedTokensData
        ? BigInt(unlockedTokensData as bigint)
        : BigInt(0);
    ...
    formatTokenAmount,
    weightTokenDecimals,
    lastSubmissionTimeData,
    rewardData,
    halfLifeSecondsData,
    quorumData,
    operationLockingPeriodData,
    withdrawalLockingPeriodData,
    alphaData,
    depositTimestampData,
    lastOperationTimestampData,
  ]);

  // Early validation moved down
  const {
    oracle,
    loading: oracleLoading,
    error: oracleError,
  } = useOracle(oracleAddress || "", chainId);
  ...
  useEffect(() => {
    if (contractError) {
      toast({
        ...
        description:
          contractError.message || "An error occurred during the transaction.",
        variant: "destructive",
      });
      setIsSubmitting(false);
      setIsDepositing(false);
  ...
  const handleSubmitValue = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          err?.message || "Failed to submit value. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleWithdrawTokens = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          err?.message || "Failed to withdraw tokens. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleVoteBlacklist = async () => {
    ...
    try {
      ...
      toast({
        title: "Vote Failed",
        description:
          err?.message || "Failed to submit blacklist vote. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleVoteWhitelist = async () => {
    ...
    try {
      ...
      toast({
        title: "Vote Failed",
        description:
          err?.message || "Failed to submit whitelist vote. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleUpdateVoteWeights = async () => {
    ...
    try {
      ...
      toast({
        title: "Update Failed",
        description:
          err?.message || "Failed to update vote weights. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  const simulateRead = useCallback(
    async (fnName: "readValue" | "readLatestValue") => {
      ...
      try {
        ...
        if (userAddress) {
          console.warn(
            `[OracleInteraction] Falling back to neutral account for ${fnName} due to address restrictions.`,
          );
        }
        ...
        <div className="container mx-auto px-6 pt-24 pb-12">
          <div className="text-center py-20">
            ...
            <p className="text-sm text-muted-foreground mb-8">
              Expected{" "}
              <code className="bg-card/50 border border-primary/30 px-2 py-1 rounded text-primary">
                oracle
              </code>{" "}
              (0x…40 chars) and{" "}
              <code className="bg-card/50 border border-primary/30 px-2 py-1 rounded text-primary">
        ...
        <div className="container mx-auto px-6 pt-24 pb-12">
          <div className="text-center py-20">
            ...
            <p className="text-xl text-foreground font-medium">
              Loading oracle details from blockchain…
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <div className="container mx-auto px-6 pt-24 pb-12">
          <div className="text-center py-20">
            ...
            <p className="text-xl text-red-400 mb-2 font-medium">
              {oracleError || "Oracle not found"}
            </p>
            <p className="text-sm text-muted-foreground mb-8">
              The oracle at address {oracleAddress} on chain {chainId} could not
              be loaded.
            </p>
            <Link
              href="/explorer"
        ...
        <div className="mb-12 text-center max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-4">
              <h1
                className="text-4xl sm:text-5xl font-medium tracking-wide bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent"
          ...
          {oracle.description && (
            <p className="text-lg text-muted-foreground mb-6 max-w-3xl mx-auto font-light">
              {oracle.description}
            </p>
          )}
            ...
            <Card className="border-border/50 bg-card/30 backdrop-blur-sm hover:bg-card/60 border-primary/20 hover:border-white transition-all duration-300 rounded-2xl">
              <CardHeader className="border-b border-border/30">
                ...
                <CardDescription className="text-muted-foreground">
                  Submit a new price value to the oracle. You must have
                  deposited tokens to participate.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="mb-4">
              ...
              <CardContent>
                <div>
                  <div className="flex items-center mb-4 justify-between p-3 bg-card/50 border border-primary/30 rounded-xl transition-all duration-300">
                    ...
                    >
                      {isReadingLatestValue || isPending || isConfirming ? (
                  ...
                  <div className="flex items-center justify-between p-3 bg-card/50 border border-primary/30 rounded-xl transition-all duration-300">
                    ...
                    >
                      {isReadingValue || isPending || isConfirming ? (
                        <Loader2 className="h-3 w-3 animate-spin" />
                      ) : (
                        "Read"
                      )}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Card className="border-border/50 bg-card/30 backdrop-blur-sm hover:bg-card/60 border-primary/20 hover:border-white transition-all duration-300 rounded-2xl">
              <CardHeader className="border-b border-border/30">
                ...
                <CardDescription className="text-muted-foreground">
                  Deposit weight tokens to participate in oracle governance and
                  earn rewards.
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 p-2 bg-card/30 border border-primary/20 rounded-lg text-xs">
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-foreground/90 font-light text-xs">
                          {depositTimestamp}
                        </div>
                      </div>
                      <div className="text-center">
            ...
            <Card className="border-border/50 bg-card/30 backdrop-blur-sm hover:bg-card/60 border-primary/20 hover:border-white transition-all duration-300 rounded-2xl">
              <CardHeader className="border-b border-border/30">
                ...
                <CardDescription className="text-muted-foreground">
                  Withdraw your deposited tokens. Note the withdrawal locking
                  period.
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Card className="border-border/50 bg-card/30 backdrop-blur-sm hover:bg-card/60 border-primary/20 hover:border-white transition-all duration-300 rounded-2xl">
              <CardHeader className="border-b border-border/30">
                ...
                <CardDescription className="text-muted-foreground">
                  Vote to blacklist or whitelist addresses. Your voting power is
                  based on your deposited tokens.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
            ...
            <Card className="border-border/50 bg-card/30 backdrop-blur-sm hover:bg-card/60 border-primary/20 hover:border-white transition-all duration-300 rounded-2xl">
              <CardHeader className="border-b border-border/30">
                ...
                <CardDescription className="text-muted-foreground">
                  Update your vote weights to reflect your current token
                  balance. Required after depositing new tokens.
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                {isConnected && (
                  <div className="p-3 bg-card/50 border border-primary/30 rounded-xl mb-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-lg font-light text-primary font-medium">
                        {parseFloat(userDepositedTokens).toFixed(4)}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <Button
                  ...
                  className="w-full bg-primary hover:bg-primary/90 text-primary-foreground border-primary/50 h-12 rounded-xl transition-all duration-300"
                >
                  {isUpdatingVoteWeights || isPending || isConfirming ? (
                    <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  ) : (
                    <Settings className="h-4 w-4 mr-2" />
                  )}
                  {isUpdatingVoteWeights || isPending
                    ? "Updating..."
                    : isConfirming
                      ? "Confirming..."
                      : "Update Vote Weights"}
          ...
          <Card className="border-border/50 bg-card/30 backdrop-blur-sm hover:bg-card/60 border-primary/20 hover:border-white transition-all duration-300 rounded-2xl">
            ...
            <CardContent className="p-6">
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm">
                  Connect your wallet to view detailed transaction history
                </p>
              </div>
            </CardContent>
          </Card>
